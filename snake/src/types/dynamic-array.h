...
#define __DYNAMIC_ARRAY_DEFINITION(element_type, struct_name) \
    ...
    static inline struct_name struct_name##CreateSized(size_t initial_size) { return (struct_name){ .elements = (element_type *)malloc(sizeof(element_type) * initial_size), .length = 0, .size = initial_size }; } \

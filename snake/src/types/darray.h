...
// Creates a darray with a custom size
#define darray_sized(element_type, initial_size) \
    ((darray_t){.length = 0, .size = initial_size, .item_size = sizeof(element_type), .items = malloc(sizeof(element_type) * initial_size)})
// Creates a darray
#define darray(element_type) darray_sized(element_type, DARRAY_DEFAULT_SIZE)

#define darray_push(darray, item)                                                 \
    do {                                                                          \
        if (darray.size == darray.length) {                                       \
            darray.size *= 2;                                                     \
            darray.items = realloc(darray.items, darray.item_size * darray.size); \
        }                                                                         \
        darray.items                                                              \
    } while (0)
...
#define __DYNAMIC_ARRAY_DEFINITION(element_type, struct_name)                                                                               \
    ...
    static inline struct_name struct_name##CreateSized(size_t initial_size) {}                                                              \
    ...
    static inline void struct_name##Add(struct_name* array, element_type element, size_t index) {                                           \
        if (index >= array->length) { index = array->length - 1; }                                                                          \
        if (array->length == array->size) {                                                                                                 \
            array->size *= 2;                                                                                                               \
            array->elements = (element_type*)realloc(array->elements, array->size);                                                         \
        }                                                                                                                                   \
        for (size_t i = index; i < array->length; ++i) { array->elements[i + 1] = array->elements[i]; }                                     \
        array->length++;                                                                                                                    \
        array->elements[index] = element;                                                                                                   \
    }                                                                                                                                       \
    static inline element_type struct_name##Remove(struct_name* array, size_t index) {                                                      \
        if (index >= array->length) { index = array->length - 1; }                                                                          \
        element_type element = array->elements[index];                                                                                      \
        for (size_t i = index; i < array->length; ++i) { array->elements[i] = array->elements[i + 1]; }                                     \
        array->length--;                                                                                                                    \
        return element;                                                                                                                     \
    }                                                                                                                                       \
 ...
 */
#define for_darray_each(element_type, var, array) for (element_type* var = array.elements; var < array.elements + array.length; var++)

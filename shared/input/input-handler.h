...
#define MAP_MOUSE_BUTTON_PRESSED(btn)  ((InputMap){METHOD_MOUSE_BUTTON_PRESSED, {btn}})
#define MAP_MOUSE_BUTTON_RELEASED(btn) ((InputMap){METHOD_MOUSE_BUTTON_RELEASED, {btn}})
#define MAP_MOUSE_BUTTON_DOWN(btn)     ((InputMap){METHOD_MOUSE_BUTTON_DOWN, {btn}})
#define MAP_MOUSE_BUTTON_UP(btn)       ((InputMap){METHOD_MOUSE_BUTTON_UP, {btn}})
...
#define MAP_GAMEPAD_BUTTON_PRESSED(btn)  ((InputMap){METHOD_GAMEPAD_BUTTON_PRESSED, {btn}})
#define MAP_GAMEPAD_BUTTON_RELEASED(btn) ((InputMap){METHOD_GAMEPAD_BUTTON_RELEASED, {btn}})
#define MAP_GAMEPAD_BUTTON_DOWN(btn)     ((InputMap){METHOD_GAMEPAD_BUTTON_DOWN, {btn}})
#define MAP_GAMEPAD_BUTTON_UP(btn)       ((InputMap){METHOD_GAMEPAD_BUTTON_UP, {btn}})

// Gamepad triggers input //

#define MAP_GAMEPAD_TRIGGER(trigr)                           ((InputMap){METHOD_GAMEPAD_TRIGGER, {.trigger = {trigr, 0}}})
#define MAP_GAMEPAD_TRIGGER_NORM(trigr)                      ((InputMap){METHOD_GAMEPAD_TRIGGER_NORM, {.trigger = {trigr, 0}}})
#define MAP_GAMEPAD_TRIGGER_THRESHOLD(trigr, threshold)      ((InputMap){METHOD_GAMEPAD_TRIGGER, {.trigger = {trigr, ftof16(threshold)}}})
#define MAP_GAMEPAD_TRIGGER_NORM_THRESHOLD(trigr, threshold) ((InputMap){METHOD_GAMEPAD_TRIGGER_NORM, {.trigger = {trigr, ftof16(threshold)}}})

// Gamepad joysticks input //

#define MAP_GAMEPAD_JOYSTICK(joystck, range)                      ((InputMap){METHOD_GAMEPAD_JOYSTICK, {.joystick = {joystck, range, 0}}})
#define MAP_GAMEPAD_JOYSTICK_THRESHOLD(joystck, threshold, range) ((InputMap){METHOD_GAMEPAD_JOYSTICK, {.joystick = {joystck, range, ftof16(threshold)}}})

// Representation of an input mapping result
typedef struct {
    bool b;  // `false` if the input is in its resting position, `true` otherwise
    f32 f;   // A continuous value of non-button inputs or `0-1` for buttons
} InputResult;
 ...
 * @return Number of invalid input mappings.
 */
action_size GreedyInputHandlerMappingsSet(GreedyInputHandler* handler, const InputMap keyboard_mappings[], const InputMap gamepad_mappings[]);
